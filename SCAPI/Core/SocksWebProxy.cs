...
namespace SCAPI.Core
{
	public class SocksWebProxy : IWebProxy
	{
		...
		///     Whether to allow bypassing the proxy server.
		///     The current implementation to allow bypassing the proxy server requiers elevated privileges.
		///     If you want to use the library in an environment with limited privileges (like Azure Websites or Azure Webjobs),
		///     set allowBypass = false
		/// </param>
		/// <returns></returns>
		...
		{
			var isSocksPortListening = IPGlobalProperties.GetIPGlobalProperties().GetActiveTcpListeners().Any(x => x.Port == Config.SocksPort);
